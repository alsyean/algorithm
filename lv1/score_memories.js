...
예를 들어 사진 속 인물의 이름이 ["may", "kein", "kain"]이고 각 인물의 그리움 점수가 [5점, 10점, 1점]일 때 해당 사진의 추억 점수는
16(5 + 10 + 1)점이 됩니다.
다른 사진 속 인물의 이름이 ["kali", "mari", "don", "tony"]이고 ["kali", "mari", "don"]의 그리움
점수가 각각 [11점, 1점, 55점]]이고, "tony"는 그리움 점수가 없을 때, 이 사진의 추억 점수는 3명의 그리움 점수를 합한 67(11 + 1 + 55)점입니다.
...
function solution(name, yearning, photo) {
    ...
    photo.forEach( (v,i)=> {
        v.forEach( (s) => {
            // 점수가 있는 사람 존재 하는지 확인
            if ( name_score.hasOwnProperty(s) === true ) {
                // 사람에 대한 점수 추가
                score += name_score[s];
            }
        })
